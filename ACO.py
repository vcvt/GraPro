...
(ALPHA, BETA, RHO, Q) = (1.0, 10, 0.8, 100.0)
(city_num, ant_num, iter_max) = (280, 10, 100)
distance_x = []
distance_y = []
for line in open('a280.tsp'):
    items = line.strip('\n').split()
...
class Ant(object):
    ...
    def __choice_next_city(self):
        ...
        for i in self.open_table_city:
            if  distance_graph[self.current_city][i] == 0:
                next_city = i
                self.open_table_city[next_city] = False
                self.current_city = next_city
                return next_city
            total_prob += pow( pheromone_graph[self.current_city][i], ALPHA ) *\
        ...
        for i in xrange(city_num):
            if  self.open_table_city[i]:
                select_citys_prob[i] = pow(pheromone_graph[self.current_city][i], ALPHA) *\
                                       pow((1.0 / distance_graph[self.current_city][i]), BETA) / total_prob
            else:
    ...
    def __init__(self):
        ...
        for i in xrange(city_num):
            for j in xrange(city_num):
                temp_distance = math.sqrt((distance_x[i] - distance_x[j]) * (distance_x[i] - distance_x[j]) +
...
if __name__ == '__main__':
    ...
    y = []
    for i in range(city_num):
        x.append(distance_x[test.best_ant.path[i]])
        y.append(distance_y[test.best_ant.path[i]])
    x.append(distance_x[test.best_ant.path[0]])
    y.append(distance_y[test.best_ant.path[0]])
    plt.plot(x,y)
    plt.show()
